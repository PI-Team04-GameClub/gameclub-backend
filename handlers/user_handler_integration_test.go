...
func TestUserHandler_CreateUser_ReturnsCreatedUser(t *testing.T) {
	// Given: A valid create user request
	db := setupTestDB(t)
	app := setupUserTestApp(db)
...
func TestUserHandler_CreateUser_DuplicateEmail(t *testing.T) {
	...
	db.Create(&models.User{FirstName: "Existing", Email: "john@example.com", Password: "hashed"})
...
func TestUserHandler_CreateUser_PasswordIsHashed(t *testing.T) {
	// Given: A valid create user request
	db := setupTestDB(t)
	app := setupUserTestApp(db)
